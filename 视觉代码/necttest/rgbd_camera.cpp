...
namespace  hitcrt{
  RGBDcamera::RGBDcamera(MODE _mode,Device rgbd, const char* target) {  ///构造函数
    m_mode = _mode;
    m_targetFile = target;
    openni::OpenNI::initialize();
    rc = openni::STATUS_OK;
    if(m_mode == RGBDcamera::ONI_mode) {
      rc = camera.open(m_targetFile);          //打开视频
